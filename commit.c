...
int commit_create(const char *message, ObjectID *commit_id_out) {
    ...
    // Step 6: Serialize commit to text format
    void *commit_data;
    size_t commit_len;
    if (commit_serialize(&commit, &commit_data, &commit_len) != 0) return -1;

    // Step 7: Write commit object to store
    ObjectID commit_id;
